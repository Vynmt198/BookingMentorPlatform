...
export async function loginWithGoogle(body, req) {
  ...
  const accessTokenParam = body?.accessToken;

  // Phải có ít nhất một trong hai: idToken (credential) hoặc accessToken
  ...
  // Nhánh 1: accessToken từ browser redirect flow (expo-auth-session trên Expo Go)
  if (!credential && accessTokenParam) {
    // Gọi Google UserInfo endpoint để lấy thông tin người dùng
    try {
      const userInfoRes = await fetch(`https://www.googleapis.com/oauth2/v3/userinfo`, {
        headers: { Authorization: `Bearer ${accessTokenParam}` },
      });
      if (!userInfoRes.ok) {
        return { ok: false, status: 401, error: "Access token Google không hợp lệ hoặc đã hết hạn." };
      }
      ...
      console.error("[auth/google] Google UserInfo fetch failed:", err?.message || err);
      return { ok: false, status: 401, error: "Không xác thực được access token Google." };
    }
  }

  // Nhánh 2: idToken / credential (native SDK hoặc mock)
  if (credential) {
    if (process.env.NODE_ENV === "development" && credential.startsWith("mock_google_token_")) {
      ...
      payload = {
        sub: "mock_sub_" + email.replace(/[^a-zA-Z0-9]/g, ""),
        email: email,
        name: email.split("@")[0].charAt(0).toUpperCase() + email.split("@")[0].slice(1) + " (Google)",
