...
function AppInner() {
  ...
  // Phân tích CV: JD (2 file) hoặc theo ngành (1 file + field)
  ...
  const triggerCvAnalysisJd = async () => {
    const resume = await pickCvDocument();
    if (!resume) return;
    notifyUser('Bước 2/2', 'Tiếp theo chọn file JD (PDF/DOCX).');
    const jd = await pickCvDocument();
    if (!jd) return;
    const fileSizeInMB = resume.size != null ? (resume.size / (1024 * 1024)).toFixed(2) : '?';
    setCvFile({ name: `${resume.name} + ${jd.name}`, size: `${fileSizeInMB} MB` });
    await runCvPipeline((onProgress) =>
      analyzeAndSaveCvWithJd(
        {
          uri: resume.uri,
