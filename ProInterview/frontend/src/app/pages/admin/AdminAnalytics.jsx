  ...
  return (
    <div className="mb-5 grid grid-cols-3 gap-2 sm:gap-3">
      ...
      <div className="rounded-xl border border-amber-100 bg-amber-50/50 px-3 py-3 text-center sm:px-4">
        <p className="text-[10px] font-black uppercase tracking-wider text-amber-700">Cuoi funnel</p>
      ...
      <div className="rounded-xl border border-emerald-100 bg-emerald-50/50 px-3 py-3 text-center sm:px-4">
        <p className="text-[10px] font-black uppercase tracking-wider text-emerald-700">Ty le</p>
  ...
  return (
    <div className={adminPageWrap}>
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={adminHeaderRow}>
        <AdminPageHeader
          kicker="Bao cao nen tang"
          title={
            <>
              <span className="text-violet-700">Phan tich</span> &amp; hanh vi
            </>
          }
          subtitle="Tong quan nguoi dung, lich hen, noi dung va hanh trinh tren ProInterview (7 ngay gan nhat)."
        />
      ...
      {loading && !stats ? (
        <div className="glass-card flex min-h-[240px] items-center justify-center p-10">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-semibold text-slate-500">Dang tai du lieu phan tich...</p>
          </div>
        </div>
      ) : (
        <>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={adminStatGrid4}>
            <KpiCard label="Khach hang" value={stats?.users ?? 0} icon={Users} color="#8037f4" tint="violet" />
            <KpiCard label="Co van" value={stats?.mentors ?? 0} icon={GraduationCap} color="#65a30d" tint="lime" />
            <KpiCard label="Lich hen" value={stats?.bookings ?? 0} icon={Calendar} color="#0284c7" tint="sky" />
            <KpiCard label="Mua khoa" value={stats?.enrollmentsPaid ?? 0} icon={BookOpen} color="#d97706" tint="amber" />
          </motion.div>
          ...
          >
            ...
            <div className="space-y-4 p-5 sm:p-7">
              ...
              <div className={adminStatGrid4}>
                <MiniStat label="Khoa cho duyet" value={stats?.courses?.pendingReview ?? 0} />
                <MiniStat label="Khoa xuat ban" value={stats?.courses?.published ?? 0} accent="text-emerald-700" />
                <MiniStat label="Phan tich CV" value={content?.cvAnalyses ?? 0} accent="text-sky-700" />
                <MiniStat
                  label="Phien AI"
            ...
            <div className="grid items-stretch gap-6 lg:grid-cols-2 xl:grid-cols-5">
              ...
              >
                <SectionHeader
                  icon={Route}
                  kicker="Hanh vi"
                  title={`Top trang · ${behavior.days} ngay`}
                  desc="Trang user o lau nhat (da dang nhap)."
                  badge={`${behavior.topRoutes?.length ?? 0} trang`}
                />
                <div className="flex-1 p-5 sm:p-7">
                  {(behavior.topRoutes || []).length === 0 ? (
                    <EmptyHint>Chua co du lieu - user can dang nhap va duyet app.</EmptyHint>
                  ) : (
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={adminGlassTable}>
              <SectionHeader icon={Calendar} kicker="Van hanh" title="Lich hen theo trang thai" />
              <div className="p-6 sm:p-8">
                {bookingBreakdown.length === 0 ? (
                  <EmptyHint>Chua co lich hen.</EmptyHint>
                ) : (
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={adminGlassTable}>
              <SectionHeader icon={Brain} kicker="AI" title="Phong van thong minh" />
              <div className={`${adminStatGrid3} p-6 sm:p-8`}>
                <MiniStat label="Tong phien" value={content?.interviewSessions ?? 0} />
                <MiniStat
                  label={interviewOps?.periodDays ? `${interviewOps.periodDays} ngay qua` : "7 ngay qua"}
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={adminGlassTable}>
            <SectionHeader
              icon={Flag}
              kicker="Gan day"
              title="Lich hen moi nhat"
              desc="Cac buoi mentor vua tao hoac cap nhat."
            />
            <div className="max-w-full overflow-x-auto overscroll-x-contain">
              <table className="w-full min-w-[720px] border-collapse text-left">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/80">
                    <th className={adminThCell}>Hoc vien</th>
                    <th className={adminThCell}>Co van</th>
                    <th className={adminThCell}>Ngay gio</th>
                    <th className={`${adminThCell} text-center`}>Buoi hen</th>
                    <th className={`${adminThCell} text-center`}>Thanh toan</th>
                    <th className={`${adminThCell} text-right`}>So tien</th>
                  </tr>
                </thead>
                  ...
                  {recentBookings.length === 0 ? (
                    <tr>
                      <td colSpan={6} className={adminTdCell}>
                        <EmptyHint>Chua co lich hen gan day.</EmptyHint>
                      </td>
                    </tr>
                  ) : (
            ...
            <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4 sm:px-8">
              <p className="text-xs font-semibold text-slate-500">{recentBookings.length} buoi hien thi</p>
              <Link
                to="/admin/bookings"
