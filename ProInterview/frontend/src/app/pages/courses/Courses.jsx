...
function LevelBadge({ level }) {
  const config = LEVEL_BADGE[level] || LEVEL_BADGE.Beginner;
  return (
    <span
    ...
    >
      <div className="relative aspect-video w-full overflow-hidden bg-violet-50">
        ...
        <div className="absolute bottom-3 left-3 flex items-center gap-2">
          ...
          />
          <p className="max-w-[140px] truncate text-[11px] font-bold text-white drop-shadow-sm">
            {course.mentorName}
          </p>
        </div>
      </div>
...
export function Courses() {
  ...
  const loadCourses = () => {
    ...
    fetchCourses()
      .then((res) => {
        if (res.success) {
          const mapped = res.courses.map((course) => {
            const { rating } = normalizeCourseStats(course.stats);
            return {
              id: course._id,
              title: course.title,
              description: course.description,
              thumbnail: mediaSrc(course.thumbnail, DEFAULT_COURSE_THUMB),
              category: course.topics?.[0] || "Kỹ năng khác",
              level: normalizeLevel(course.level),
              mentorName: course.mentorId?.userId?.name || "Khuất danh",
              mentorAvatar: avatarSrc(course.mentorId?.userId?.avatar),
              mentorTitle:
                course.mentorId?.title ||
                course.mentorId?.userId?.desiredPosition ||
                "",
              mentorCompany:
                course.mentorId?.company ||
                course.mentorId?.userId?.currentCompany ||
                "",
              rating,
              duration: course.totalDurationMinutes || 0,
  ...
  const filteredCourses = useMemo(() => {
    ...
    return [...courses]
      .filter((course) => {
        ...
        if (!courseMatchesTopic(course.category, selectedCategory || null)) {
          return false;
        }
        if (selectedFee === "free" && course.price !== 0) return false;
        if (selectedFee === "paid" && course.price === 0) return false;
        if (
          q &&
          !course.title.toLowerCase().includes(q) &&
          !course.description?.toLowerCase().includes(q) &&
          !course.mentorName.toLowerCase().includes(q)
        ) {
          return false;
        }
        return true;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, selectedLevel, selectedCategory, selectedFee, sortBy]);
    ...
    <MentorPageShell bottomPad="pb-20" showAmbient={false}>
      ...
      >
        ...
        <div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center px-4">
          ...
          >
            Các khóa học do Mentor xây dựng, giúp bạn bổ sung kỹ năng cần thiết
            cho hành trình ứng tuyển.
          </motion.p>
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-transparent py-3 pl-11 pr-4 text-xs font-semibold text-white outline-none placeholder:text-slate-400"
              />
            </div>
            {searchQuery ? (
              <button
        ...
        <div className={`${CUSTOMER_SHELL_MAX} w-full`}>
                ...
                <div className="grid grid-cols-1 items-end gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <AppSelect
                      ...
                      triggerClassName="h-[38px] w-full rounded-2xl !border-violet-800 !bg-violet-950/40 text-xs font-bold !text-slate-100 hover:!bg-violet-900/30 focus:!border-[#8037f4] focus:ring-2 focus:ring-violet-500/20"
                    />
                  </div>
          ...
          {!loading && !error && filteredCourses.length === 0 ? (
            <div className="rounded-3xl border border-violet-100 bg-white py-16 text-center shadow-sm">
              ...
              <h3 className="mb-2 text-lg font-black text-violet-950">
                Không tìm thấy khóa học
              </h3>
              <p className="mb-7 text-sm text-slate-500">
                {searchQuery ? `Không có kết quả cho "${searchQuery}". ` : ""}
                Thử đổi bộ lọc khác nhé.
              </p>
              <motion.button
                type="button"
                onClick={clearFilters}
