  ...
  return (
    <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
      <SheetContent className="w-full sm:max-w-md flex flex-col h-full bg-white">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2 text-[#8037f4]">
            <ShoppingCart className="w-5 h-5 text-[#8037f4]" />
            Giỏ hàng của bạn
          </SheetTitle>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto py-4 mt-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              <div className="w-16 h-16 rounded-full bg-[#8037f4]/10 flex items-center justify-center mb-4">
                <ShoppingCart className="w-7 h-7 text-[#8037f4]" />
              </div>
              <p className="font-medium text-gray-700">Giỏ hàng đang trống</p>
              <p className="text-sm text-gray-400 mt-1">Thêm khóa học hoặc buổi mentor bạn quan tâm để thanh toán nhanh hơn.</p>
              <Button
                className="mt-5 bg-[#8037f4] hover:bg-[#6c2bd6] text-white"
